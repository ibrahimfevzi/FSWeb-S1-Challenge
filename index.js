...
return `${first.indexOf(first[second])}. indekste bulunan fenomen: ${ first[second].profile}`;
}
...
function fenomenEkle(bir, sıraNo, profilIsmi, takipçiSayisi, gönderimSayisi, platformAdi) {
  let yeniDizi = [...bir];
  let nesne ={
    "number": 6,
    "profile": "Workintech",
...
function platformaGoreCokGonderiYapanFenomen(dizi9, platform9) {
  let maxPost = 0;
  let enFenomen = " ";
  for (let i = 0; i < dizi9.length; i++) {
    if (
      dizi9[i].platform == platform9 &&
      dizi9[i].posts != "NA" &&
      dizi9[i].posts > maxPost
    ) {
      enFenomen = dizi9[i].profile;
      maxPost = dizi9[i].posts;
    }
  }
  return enFenomen;
}
